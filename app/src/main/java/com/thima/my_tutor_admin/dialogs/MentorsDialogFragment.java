...
import com.thima.my_tutor_admin.adapters.MentorsAdapter;
...
public class MentorsDialogFragment extends DialogFragment {
    ...
    private final List<MentorsModel> Items = new ArrayList<>();
                    ...
                    public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                        if(!value.isEmpty()){
                            for (DocumentChange dc : value.getDocumentChanges()){
                                switch (dc.getType()) {
                                    case ADDED:
                                        MentorsModel mentor = dc.getDocument().toObject(MentorsModel.class);
